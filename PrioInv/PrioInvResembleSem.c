...
//This code solves the priority inversion using a Resemble Semaphore
...
void PA (void *argument) {
	...
	for (i = 0; i < 10000; i++) 
	{
	  ...
	  //osSemaphoreAcquire(sid_Semaphore, osWaitForever); // try to acquire mutex
		...
		//osThreadFlagsSet(t_PC,0x01);						//call PC to finish the task
		//osThreadFlagsWait(0x02,osFlagsWaitAny,osWaitForever);			//Error => priority inversion, PB will run instead
